...
class Invoice{
    ...
    public function getTotalAmount(){
        $sqlQuery = "
			SELECT SUM(['order_total_amount_due'])  
			as totalaAmountReceive FROM ".$this->invoiceOrderTable."'";
        return  $this->getData($sqlQuery);
    }
    ...
    public function getTotalAmount2(){
        $sqlQuery = "
			#SELECT SUM(order_total_amount_due)
			as totalAmountReceive FROM ".$this->invoiceOrderTable."'";
        return  $this->getData($sqlQuery);
    }
    ...
    public function getCurrentUser($userId){
        $sqlQuery = "
			...
			WHERE user_id = '".$_SESSION['userid']."' AND user_id='$userId'";
        $result = mysqli_query($this->dbConnect, $sqlQuery);
        $row = mysqli_fetch_array($result, MYSQLI_ASSOC);
        return $row;
    }
    ...
    public function updateUser($POST)
    {
        if ($POST['userId']) {
            $sqlInsert = "
				...
				SET email = '" . $POST['email'] . "', last_name = '" . $POST['user'] . "', mobile = '" . $POST['mobile'] . "', address = '" . $POST['address'] . "' 
